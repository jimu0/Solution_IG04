...
#pragma warning restore CA1819 // 属性不应返回数组
        ...
        public void WriteString(string? x)
        {
            ...
            if (n <= 0) return;
            EnsureWrite(n);
            if (x != null) Encoding.UTF8.GetBytes(x, 0, x.Length, Bytes, WriterIndex);
            WriterIndex += n;
        }

        // byte[], [start, end)
        public static Func<byte[], int, int, string>? StringCacheFinder => null;
        ...
        public string ReadString()
        {
            ...
            EnsureRead(n);
            string s = StringCacheFinder == null ? Encoding.UTF8.GetString(Bytes, ReaderIndex, n) : StringCacheFinder(Bytes, ReaderIndex, n);
            ReaderIndex += n;
            return s;
        }
